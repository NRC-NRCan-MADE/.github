# NRC - NRCan MAD3E 
This GitHub Organization is the dedicated code sharing space for MAD3E.

## What is MADE?
The Materials Acceleration Demonstration Ecosystem (MADE) is a joint National Research Council of Canada (NRC) and Natural Resources Canada (NRCan) program dedicated to developing accelerated materials discovery tools (including self-driving labs) for clean energy applications. The goal of the program is to help Canada meet its emission reduction targets and grow its clean energy and manufacturing sectors.

Co-located in the TerraCanada Mississauga and Hamilton facilities, MADE is home to a team of 20+ research, technical, and support staff from both the NRC and NRCan, emphasizing a multidisciplinary approach to solving critical energy problems.

[Read more about MAD3E](https://science.gc.ca/site/science/en/laboratories-canada/science-hubs/terracanada-science-and-innovation-terracanada/what-made)

## Policy and Guidelines
Please read the policy and guidelines first: [Interdepartmental_LoA_MADE MoU_NRC_NRCan](https://github.com/NRC-NRCan-MADE/.github/blob/main/Interdepartmental_LoA_MADE%20MoU_NRC_NRCan_A0043925_signed%20JFH.pdf)

## Repository Management
Repositories are to be categorized as development, production, or public. Before you create a repository, please get in touch to discuss the stage of usage.
* **Development**: Private repo. Unstable release. At this stage, repositories might just be hosted in individual organization’s code repository platforms.
* **Production**: Private repo. Stable release that is ready to be shared with project members.
* **Public**: Public repo. both parties have made consensual decision to publicize and have been approved for publication. 

## Repository Creation
Only the admin users hold the right to create a new repository. If you want to create a new repository, please get in touch with Mike or Keli. Contact information below.

## Admin Users
* NRCan: Michael Greenwood <michael.greenwood@nrcan-rncan.gc.ca>
* NRC: Keli Chiu <keli.chiu@nrc-cnrc.gc.ca>
